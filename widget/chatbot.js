(function () {
  ...
  var css = `
    ...
    #cb-input-row { display: flex; align-items: center; gap: 8px; padding: 12px 16px; border-top: 1px solid var(--cb-border); }
    #cb-input {
    ...
    #cb-send:not(:disabled):hover { filter: brightness(1.08); }
    ...
    @media (prefers-reduced-motion: reduce) {
      ...
      .cb-typing span, .cb-msg, #cb-launcher svg { animation: none; }
    }
  `;
  ...
  var CLOSE_ICON = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>';
  ...
  // Swapea el icono del launcher (chat ↔ cerrar) preservando el badge si seguía visible.
  function setLauncherIcon(icon) {
    var badge = launcherEl.querySelector('#cb-badge');
    // Sacamos la clase y forzamos un reflow antes de re-agregarla para que la
    // animación cb-icon-in vuelva a dispararse en cada toggle (si no, solo corría la 1ra vez).
    launcherEl.classList.remove('cb-swap');
    void launcherEl.offsetWidth;
    launcherEl.innerHTML = icon;
    if (badge) launcherEl.appendChild(badge);
    launcherEl.classList.add('cb-swap');
  }
  function openPanel() {
    panelEl.classList.add('open');
    panelEl.setAttribute('aria-modal', 'true');
    launcherEl.setAttribute('aria-expanded', 'true');
    launcherEl.setAttribute('aria-label', 'Cerrar chat');
    var b = document.getElementById('cb-badge');
    if (b) b.remove();
    setLauncherIcon(CLOSE_ICON);
